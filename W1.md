...
- offsetHeight 为元素高度包括 padding，也可直接用 height
- innerHeight 为 窗口的高度
...
- 绝对定位的 100%包括 border 大小，其他则只包括内容。（起始位置都从父元素的 content 左上角开始
...
> u 开启完整的 unicode 支持。该修饰符能够修正对于代理对的处理。更详细的内容见章节 unicode 标记。  
